...
type tokenCredentialResolver func(credentialRequest) (string, error)
...
func newTokenCredentialRequest(
	...
	if appID == 0 && clientID == "" {
		return credentialRequest{}, fmt.Errorf("must specify either --app-id or --client-id")
	}
	if appID != 0 && clientID != "" {
		return credentialRequest{}, fmt.Errorf("cannot use both --app-id and --client-id")
	}
...
func resolveTokenCredential(request credentialRequest) (string, error) {
...
func resolveTokenCredentialWith(request credentialRequest, mint tokenMinter) (string, error) {
	cfg, err := config.Load()
	if err != nil {
	...
	_, tokenTarget, err := credentialTarget(*selectedApp, request.Repository)
	if err != nil {
		return "", err
	}
