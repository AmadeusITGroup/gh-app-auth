...
func TestNewDefaultLoader(t *testing.T) {
	...
	// Should contain .config/gh/app-auth.yml
	if !containsString(configPath, ".config/gh/app-auth.yml") {
		t.Errorf("NewDefaultLoader() config path = %v, want path containing .config/gh/app-auth.yml", configPath)
	}
}
...
func TestGetDefaultConfigPath_WithEnvVar(t *testing.T) {
	...
	expectedPath := "/custom/config/path.yml"
	os.Setenv("GH_APP_AUTH_CONFIG", expectedPath)

	path := getDefaultConfigPath()
	if path != expectedPath {
		t.Errorf("getDefaultConfigPath() with env var = %v, want %v", path, expectedPath)
	}
}
