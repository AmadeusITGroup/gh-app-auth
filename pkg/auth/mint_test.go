...
func TestHTTPClientStopsAfterRedirectLimit(t *testing.T) {
	client := newHTTPClient()
	from := httptest.NewRequest(http.MethodGet, "https://api.github.com/app", nil)
	to := httptest.NewRequest(http.MethodGet, "https://api.github.com/app?page=11", nil)
	...
	err := client.CheckRedirect(to, via)
	if err == nil || !strings.Contains(err.Error(), "stopped after") {
		t.Fatalf("CheckRedirect() error = %v, want redirect limit error", err)
	}
}
