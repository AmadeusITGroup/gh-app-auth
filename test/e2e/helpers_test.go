...
// isolatedAppConfig returns an env for an isolated gh-app-auth
// configuration. Combines git isolation and app config isolation.
func isolatedAppConfig(t *testing.T) []string {
	t.Helper()
	configDir := t.TempDir()
...
// RunCmdInDir executes the binary with the working directory set — needed
// for commands that inspect the current repository (e.g. exec without --repo).
func RunCmdInDir(t *testing.T, env []string, dir string, args ...string) (string, string, error) {
	t.Helper()
	cmd := exec.Command(binaryPath(t), args...)
	cmd.Env = env
	cmd.Dir = dir
...
// setupSingleApp configures one GitHub App entry for the org's wildcard
// pattern (github.com/<org>/*) and returns the isolated environment.
// Uses --use-filesystem since CI has no OS keyring.
func setupSingleApp(t *testing.T, org, name string) []string {
	t.Helper()
	env := isolatedAppConfig(t)
	keyFile := writePrivateKeyFile(t, globalConfig.PrivateKeyPEM)

	pattern := fmt.Sprintf("github.com/%s/*", org)
	...
	if err != nil {
		t.Fatalf("setup for %s failed: %v\nstdout: %s\nstderr: %s", org, err, stdout, stderr)
	}
	return env
}
